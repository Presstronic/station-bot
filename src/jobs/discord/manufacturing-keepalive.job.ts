  ...
  const task = cron.schedule(
    ...
    async () => {
      try {
        ...
        if (!cachedGuild) {
          logger.warn('[manufacturing] Keep-alive: guild not in client cache; skipping', { guildId });
          return;
        }

        const thread = await cachedGuild.channels.fetch(createOrderThreadId).catch((error: unknown) => {
